...
class RiskScorer:
    ...
    def __init__(self):
        # Poids par règle (importance relative)
        self.rule_weights = {
            'PASSWORD_REQUEST': 10,
    ...
    def calculate_score(self, detection_results: Dict) -> Tuple[int, Dict]:
        ...
        # Calcul du score: (somme des poids déclenchés / somme max) * 100
        # Capped à 100
        score = min(int((triggered_weight / total_weight) * 100) if total_weight > 0 else 0, 100)
